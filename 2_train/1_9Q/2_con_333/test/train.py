...
def Train_Pyt(bfe,img):
  ...
  for i in range (n_batch): # n_batch
    ...
    pred=torch.matmul(Con_Unitary(Par1[2],S_Ker1),pred).reshape(2,4,4,16,batch).permute(4,2,0,3,1).reshape(batch,8,64) # 012 345678
...
def Pred_Pyt(img,par1,par2):
  ...
  for i in range (n_batch):
    ...
    pred=torch.matmul(cm2,pred).reshape(2,4,4,16,batch).permute(4,2,0,3,1).reshape(batch,8,64) # 012 345678
    pred=pred.pow(2).sum(dim=2)
